...
public class Flower3
{
    ...
    public Flower3? parent;
    public List<Flower3> trail = new List<Flower3>();

    int[] pieceValues = { 0, 100, 300, 300, 500, 900, 10000 };
    int[] pieceDefensiveValues = { 5, 10, 30, 30, 50, 90, 0 };

    public Flower3(Board board, Move lastMove, Flower3? parent = null)
    {
    ...
    public void CreateTrail(Board board)
    {
        ...
        foreach (Move move in board.GetLegalMoves()) {
            this.trail.Add(new Flower3(board, move, this));
        }
    }
    ...
    public int Visit(Board board, int age)
    {
        ...
        if (age > 0 && this.trail.Count > 0) {
            Random rng = new();
            int pollenDownTheTrail = this.trail[rng.Next(this.trail.Count)].Visit(board, age);

            if (this.weAreWhite != board.IsWhiteToMove) {
    ...
    public int Harvest(Board board, Move move, bool recursive = false)
    {
       int piecesScore = BitboardHelper.GetNumberOfSetBits(board.GetPieceBitboard(PieceType.Pawn, true)) * pieceValues[1] +
        ...
        if (this.isWhiteToMove) {
            return piecesScore + capPieceValue - sacrificedPieceValue + enPassantScore + castlingScore + promotionScore + queenCaptureScore + pawnMoveScore + moveToTheMiddleScore;
    ...
    public Flower3 FollowTheQueen(Board board, int depth=2)
    {
        // follow the queen
        if (depth > 0) {
            foreach (Flower3 flower in this.trail) {
            ...
            return new Flower3(board, new Move(), null);
        }
    ...
    {
        if (this.trail.Count > 0) {
            Flower3 prettiestFlower = this.trail[0];
            int highestScore = prettiestFlower.nectar;
            foreach (Flower3 flower in this.trail)
            {
                Console.WriteLine("Flower3: " + flower.lastMove.ToString() + ": " + flower.nectar);
                if (this.isWhiteToMove) {
...
public class RandomMinMax : IChessBot
{
    ...
    public void MoveTheHive(Board board)
    {
        if (this.hive == null) {
            ...
            this.hive = new Flower3(board, new Move());
        } else {
